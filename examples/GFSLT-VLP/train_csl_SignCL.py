...
# cl_criterion = signcl.SignInfoNCE()
...
def get_args_parser():
    ...
    parser.add_argument('--warmup-epochs', type=int, default=70, metavar='N',
    ...
    # * contrast learning decay
    parser.add_argument('--warmup_epochs', type=int, default=30, metavar='N',
                        help='epochs to warmup LR, if scheduler supports')
    parser.add_argument('--decay_rate', '--decayr', type=float, default=0.01, metavar='RATE',
                        help='LR decay rate (default: 0.1)')
    parser.add_argument('--decay_interval', type=int, default=10, metavar='N',
                        help='epochs to warmup LR, if scheduler supports')
...
def main(args, config):
    ...
    if args.finetune:
        ...
        def can_require_grad(tensor):
            ...
            # return False #relative with B@4
        ...
        for k, v in state_dict['model'].items():
            if 'conv_2d' in k or 'conv_1d' in k:
                ...
                new_state_dict[k] = v
            if 'trans_encoder' in k and ".embed_tokens" not in k:  # imageCLIP ->  mytran
    ...
    if args.eval:
        ...
        test_stats = evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX,
                              SPECIAL_SYMBOLS, PAD_IDX, device, split="dev")
        ...
        test_stats = evaluate(args, test_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX,
                              SPECIAL_SYMBOLS, PAD_IDX, device, split="test")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX,
                              SPECIAL_SYMBOLS, PAD_IDX, device, epoch=epoch, cl_decay=cl_decay)
        print(f"BELU-4 of the network on the {len(dev_dataloader)} dev videos: {test_stats['belu4']:.2f}")

        if max_accuracy < test_stats["belu4"]:
            if test_stats["belu4"] - last_acc > 1.0:
                last_acc_epoch = epoch
                last_acc = test_stats["belu4"]
        ...
        if epoch - last_acc_epoch > 10:
            cl_decay = cl_decay * args.decay_rate
            last_acc_epoch = epoch
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if epoch < args.warmup_epochs:
            ...
            total_loss += tgt_loss + 0.001 * cl_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX, SPECIAL_SYMBOLS,
             PAD_IDX, device, epoch=999, cl_decay=1.0, split="dev"):
    model.eval()
    ...
    with torch.no_grad():
        ...
        total_loss = 0.0
        total_cl_loss = 0.0
        for step, (src_input, tgt_input, _) in enumerate(metric_logger.log_every(dev_dataloader, 10, header)):
            ...
            margin = max(10, int((frames_feature.shape[1] // tgt_input['input_ids'].shape[1] + 1) * 2.3)) * 2
            num_negative = 30
            margin = min(margin, int((frames_feature.shape[1] - num_negative) / 2))
            cl_loss = cl_criterion(frames_feature, margin=margin)
            cl_v = cl_loss.item() if cl_loss.item() != None else 0
            total_cl_loss += cl_v

            metric_logger.update(loss=total_loss.item())
            metric_logger.update(cl_loss=cl_loss.item())
            metric_logger.update(cl_decay_rate=args.decay_rate)
    ...
    if utils.is_main_process() and utils.get_world_size() > 0 and args.eval:
        with open(args.output_dir + f'/tmp_pres_{split}.txt', 'w') as f:
            for i in range(len(tgt_pres)):
                f.write(tgt_pres[i] + '\n')
        with open(args.output_dir + f'/tmp_refs_{split}.txt', 'w') as f:
        ...
        # utils.get_results(tgt_pres, tgt_refs)
