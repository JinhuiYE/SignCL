...
def get_args_parser():
    ...
    parser.add_argument('--warmup_epochs', type=int, default=70, metavar='N',
                        help='epochs to warmup LR, if scheduler supports')
    parser.add_argument('--decay_rate', '--decayr', type=float, default=0.5, metavar='RATE',
                        help='LR decay rate (default: 0.1)')
    ...
    parser.add_argument('--config', type=str, default='./configs/config_gloss_free_dsl.yaml')
...
def main(args, config):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if max_accuracy < test_stats["belu4"]:
            if test_stats["belu4"] - last_acc > 2.0:
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX, SPECIAL_SYMBOLS,
             PAD_IDX, device, epoch=999, cl_decay=1.0):
    model.eval()
